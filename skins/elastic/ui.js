...
function rcube_elastic_ui() {
  ...
  function setup() {
    ...
    if (
      is_framed &&
      !rcmail.env.extwin &&
      !parent.$(".ui-dialog:visible").length
    ) {
      if (
        (title = $("h1.voice")
          .first()
      ...
      $("#compose-options")
        ...
        .each(function() {
          ...
          $(this)
            ...
            .on("change", function() {
              hidden.val(
                this.type != "checkbox" || this.checked ? $(this).val() : ""
              );
            })
            .change();
        });
    ...
    $("#taskmenu > a").each(function() {
      if (/button-([a-z]+)/.test(this.className)) {
        ...
        if (button && (data = button.data)) {
          if (data.sel) {
            data.sel =
              data.sel.replace("button-selected", "selected") + " " + name;
          }
    ...
    $(".treelist").each(function() {
      var list = this,
        callback = function() {
          $(list)[
            $(".treetoggle", list).length > 0 ? "removeClass" : "addClass"
          ]("notree");
        };

      if (window.MutationObserver) {
        new MutationObserver(callback).observe(list, {
          childList: true,
          subtree: true
        });
      }
  ...
  function create_cloned_button(target, menu_button, add_class, always_active) {
    ...
    if (!menu_button) {
      ...
      button.data({
        popup: popup,
        "toggle-button": target.data("toggle-button")
      });
      popup_init(button[0]);
      click = false;
    ...
    button
      .attr({ id: button_id, href: "#", class: btn_class })
    ...
    if (is_framed && !menu_button) {
      ...
      frame_buttons.push(
        $.extend({ button_id: button_id }, find_button(target[0].id))
      );
    } else {
      // Register the button to get active state updates
      register_cloned_button(
        target_id,
        button_id,
        btn_class.replace(" disabled", "")
      );
    }
  ...
  function layout_init() {
    ...
    $(window).on("resize", function() {
      ...
      env.resize_timeout = setTimeout(function() {
        resize();
      }, 25);
    });
    ...
    if ((layout.list.length || layout.content.length) && is_mobile()) {
      ...
      $("[data-fab]").each(function() {
        ...
        if (
          (task == "*" || task == rcmail.env.task) &&
          (action == "*" ||
            action == rcmail.env.action ||
            (action == "none" && !rcmail.env.action))
        ) {
  ...
  function init() {
    /**
     * created by al(2020-01-26)
     */
    var accept_cookie = rcmail.get_cookie("accept_cookie");
    if(accept_cookie == null){
      $("#dialog").css("display", "block");
      var footer = '<div id="dialog_footer">For more information, see our <a href="./privacy.html" target="_blank" >Privacy Notice</a></div>';
      $( "#dialog" ).dialog({
        autoOpen: true,
        modal: true,
        width: 600,
        ...
        buttons: {
          Accept: function() {
            ...
            rcmail.set_cookie("accept_cookie", "accept", exp);
            if(rcmail.get_cookie("accept_cookie")){
              $( this ).dialog( "close" );
            }
          }
        },
    ...
    $("#password_toggle").mousedown(function(){
      $("#password_toggle").removeClass('eye');
      $("#password_toggle").addClass('eye-visible');
      $("#rcmloginpwd").attr("type", "text");
    });
    $("#password_toggle").mouseup(function(){
      $("#password_toggle").removeClass('eye-visible');
      $("#password_toggle").addClass('eye');
      ...
      .each(function() {
        ...
        if (rcmail[list] && rcmail[list].multiselect) {
          ...
          if (!toolbar.length) {
            ...
            (button = toolbar.find("a.select").data("toggle-button"))
          ) {
            button = $("#" + button);
          }
          ...
          if (!button) {
            button = $("<a>")
              ...
              .on("click", function() {
                if ($(this).is(".active")) table.toggleClass("withselection");
              })
            ...
            if (toolbar.is(".menu")) {
              ...
              if (layout.content) {
                var button2 = create_cloned_button(
                  button,
                  true,
                  "hidden-big hidden-large"
                );
                $('<li role="menuitem">')
                  .append(button2)
                  .appendTo("#toolbar-menu");
    ...
    rcmail
      .addEventListener("fileappended", function(e) {
        if (e.attachment.complete) {
          ...
          if (
            e.attachment.mimetype == "text/vcard" &&
      ...
      .addEventListener("managesieve.insertrow", function(o) {
    ...
    if (rcmail.task == "mail") {
        ...
        if (window.MutationObserver) {
          var observer,
            ...
            status_callback = function() {
              compose_status("attach", list.children().length > 0);
            };
      ...
      if (
        !rcmail.env.extwin &&
        (rcmail.env.action == "compose" || rcmail.env.action == "show")
      ) {
    ...
    var func,
      format = rcmail.env.date_format_localized;
    if (format) {
      func = function(input) {
        $(input)
          .filter(".datepicker")
  ...
  function bootstrap_style(context) {
    ...
    $(
      "input.button.mainaction,button.primary,button.mainaction",
      context
    ...
    // Forms
    var supported_controls =
      "input:not(.button,.no-bs,[type=button],[type=radio],[type=checkbox]),textarea";
      ...
      $(this)
        ...
        .each(function() {
          ...
          if (cells.length == 2) {
            ...
            /**custome by al**/
            if (last.find("select").length) {
              var hidden_sel_ids = [
                "rcmfd_refresh_interval",
                "rcmfd_show_images"
              ];
              var hidden_sel_comp_ids = [
                "rcmfd_htmleditor",
                "rcmfd_autosave",
                "rcmfd_reply_mode",
                "rcmfd_forward_attachment",
                "rcmfd_reply_all_mode"
              ];
              var sel_box = last.children("select");
              var sel_box_id = sel_box.attr("id");
              if (
                hidden_sel_ids.includes(sel_box_id) ||
                hidden_sel_comp_ids.includes(sel_box_id)
              ) {
            ...
            if (
              last.find("[type=checkbox]").length == 1 &&
              !last.find(".proplist").length
            ) {
              /**custome by al**/
              var checkbox = last.children("[type=checkbox]");
              var hidden_chk_ids = [
                "rcmfd_message_extwin",
                "rcmfd_message_show_email",
                "rcmfd_htmlmsg"
              ];
              var hidden_comp_ids = [
                "rcmfdcompose_extwin",
                "rcmfd_mdn_default",
                "rcmfd_dsn_default",
                "rcmfd_reply_same_folder"
              ];
              var chkbox_id = checkbox.attr("id");
              if (
                hidden_chk_ids.includes(chkbox_id) ||
                hidden_comp_ids.includes(chkbox_id)
              ) {
                row.css("display", "none");
              }
              /******/
              ...
              !last.find("input:not([type=hidden]),textarea,radio,select")
                .length
            ) {
            ...
            if (
              last.children(".datepicker") &&
              last.children("input").length == 2
            ) {
    ...
    $("fieldset.propform:not(.groupped) div.row", context).each(function() {
      var has_input =
        $("input:not([type=hidden]),select,textarea", this).length > 0;
      ...
      $(this)
        .children()
        .last()
        .addClass("col-sm-8" + (!has_input ? " form-control-plaintext" : ""));
      $(this)
        .children()
        .first()
    ...
    /**custome by al**/
    //hidden advanced options -composing messages
    var target = $("#rcmfd_param_folding").closest("fieldset.advanced");
    ...
    $("form.tabbed,div.tabbed", context).each(function(idx, item) {
      var tabs = [],
        nav = $("<ul>").attr({ class: "nav nav-tabs", role: "tablist" });
      ...
      $(this)
        ...
        .each(function(i, fieldset) {
          ...
          tab = $("<li>")
            ...
            .append(
              $("<a>")
                .addClass("nav-link" + (tab_class ? " " + tab_class : ""))
    ...
    $("input[type=file]:not(.custom-file-input)", context).each(function() {
      var label_text = rcmail.gettext(
          "choosefile" + (this.multiple ? "s" : "")
        ),
        label = $("<label>")
          .attr({
      ...
      .filter(function() {
        ...
        return (
          !$(this)
            ...
            .is(".propform") &&
          !$(this).parents(
            ".message-htmlpart,.message-partheaders,.boxinformation,.raw-tables"
          ).length
        );
      })
      .each(function() {
    ...
    $(
      "input.pretty-checkbox, .propform input[type=checkbox], .form-check input, .popupmenu.form input[type=checkbox], .menu input[type=checkbox]",
      context
    ).each(function() {
      pretty_checkbox(this);
    });
    ...
    $(".input-group-combo > select", context)
      .first()
      .on("change", function() {
        var select = $(this),
          fn = function() {
            select[select.next().is(":visible") ? "removeClass" : "addClass"](
              "alone"
            );
          };
    ...
    $("#message-objects", context)
      ...
      .each(function() {
        ...
        var cl =
          String(
            $(this)
              .removeClass("notice part-notice")
              .attr("class")
          ).split(/\s/)[0] || "warning";
        alert_style(this, cl);
    ...
    if (rcmail.env.task == "login" && context == document) {
      ...
      $("#login-form table tr").each(function() {
        var input = $("input,select", this),
          ...
          icon = $("<i>").attr(
            "class",
            "input-group-text icon " + input.attr("name").replace("_", "")
          );
          // if(input.attr("name") == "_pass"){
          //   var icon_apend = $("<i>").attr(
          //     "class",
          //     "input-group-text icon eye"
          //   );
          // }
          // else{
          //   var icon_apend = "";
        ...
        input
          .addClass(input.is("select") ? "custom-select" : "form-control")
          .attr("placeholder", label.text())
          .before($('<span class="input-group-prepend">').append(icon))
          // .after($('<span class="input-group-append">').append(icon_apend))
          .parent()
  ...
  function tinymce_style(elem) {
    ...
    if ($(elem).is(".mce-window")) {
      ...
      body
        .find("button")
        .filter(function() {
          return $(this).parent(".mce-btn").length > 0;
        })
      ...
      $(elem)
        .find(".mce-form > .mce-container-body")
        .each(function() {
      ...
      if (!is_mobile()) {
        ...
        if (height < max_height) {
          ...
          $(elem).css(
            "top",
            ($(window).height() - $(elem).height()) / 2 + "px"
          );
        }
      }
    }
      ...
      if (window.MutationObserver) {
        var callback = function() {
          ...
          if (!$(".mce-menu:visible").length) {
            ...
            $("<div>")
              .attr("class", "popover-overlay mce-overlay")
              .appendTo("body")
              .click(function() {
                $(this).remove();
              });
  ...
  function dropdowns_init() {
    ...
    rcube_webmail.set_iframe_events({
      mousedown: popups_close,
      touchstart: popups_close
    });
  }
  ...
  function content_frame_navigation(href, event) {
    ...
    var node,
      uid,
      list,
      _list = $("[data-list]", layout.list).data("list");

    if (!_list || !(list = rcmail[_list])) {
      // hide navbar if there are no visible buttons, e.g. Help plugin UI
      if (
        $(env.frame_nav).is(".hide-nav-buttons") &&
        !$(".buttons", env.frame_nav).children().length
      ) {
    ...
    var prev,
      ...
      next_button = $("a.button.next", env.frame_nav)
        .off("click")
        .addClass("disabled"),
      prev_button = $("a.button.prev", env.frame_nav)
        .off("click")
        .addClass("disabled");
            ...
            callback = function(data) {
              $.each(data, function(i, v) {
                ...
                if (v.type == "attributes") {
                  ...
                  if (is_framed) {
                    win
                      .$("select.ui-datepicker-month", v.target)
                      .on("change", function() {
                    ...
                    win
                      .$("select.ui-datepicker-year", v.target)
                      .on("change", function() {
                        $.datepicker._selectMonthYear(
                          $.datepicker._lastInput,
                          this,
    ...
    if (is_framed) {
      $.each(frame_buttons, function(i, button) {
        if (args.command == button.command) {
          parent
            .$("#" + button.button_id)
            [args.status ? "removeClass" : "addClass"]("disabled");
        }
      });
    }
  ...
  function screen_mode() {
    ...
    if (width <= 480) size = "phone";
    else if (width > 1200) size = "large";
    else if (width > 768) size = "normal";
    else size = "small";
  ...
  function resize() {
    ...
    $.each(content_buttons, function() {
      $(this)[mobile ? "hide" : "show"]();
    });
  }
  ...
  function screen_resize_html() {
    ...
    if (html[0].className.match(/layout-([a-z]+)/)) {
      if (RegExp.$1 != meta.mode) {
        html.removeClass("layout-" + RegExp.$1).addClass("layout-" + meta.mode);
      }
    } else {
  ...
  function screen_resize_normal() {
    ...
    if (layout.list.length) {
      show =
        layout.list.is(env.last_selected) ||
        (!layout.sidebar.is(env.last_selected) &&
          !layout.sidebar.is(".layout-sticky"));
  ...
  function screen_resize_large() {
    $.each(layout, function(name, item) {
  ...
  function screen_resize_small_all() {
    ...
    if (layout.sidebar.length) {
      show =
        !got_content &&
        (layout.sidebar.is(env.last_selected) || !layout.list.length);
      layout.sidebar[show ? "removeClass" : "addClass"]("hidden");
    }
  ...
  function searchbar_init(bar) {
    var unread_button = $(),
      ...
      update_func = function() {
        ...
        unread_button[
          rcmail.gui_objects.search_filter &&
          $(rcmail.gui_objects.search_filter).val() == "UNSEEN"
            ? "addClass"
            : "removeClass"
        ]("selected");
      };
    ...
    input.on("input change", update_func).on("focus blur", function(e) {
      input.attr("placeholder", e.type == "blur" ? placeholder : "");
    });
    ...
    $("a.reset", bar).on("click", function(e) {
      ...
      input
        .val("")
        .change()
        .trigger("keyup.treelist", { keyCode: 27 });
      if ($(bar).is(".open")) {
        options_button.click();
      }
    ...
    rcmail
      .addEventListener("init", update_func)
  ...
  function toolbar_init() {
    ...
    var list_mark,
      ...
      button_func = function(button, items, cloned) {
        var item = $('<li role="menuitem">'),
          button = cloned
            ? create_cloned_button($(button), true, "hidden-big hidden-large")
            : $(button).detach();
    ...
    if (items.length) {
      var container = layout.content.children(".header"),
        ...
        menu_button = $(
          '<a class="button icon toolbar-menu-button" href="#menu">'
    ...
    $(item)
      ...
      .on("show.bs.popover", function(event) {
        ...
        if (!is_mobile()) {
          ...
          popup.css(
            "max-height",
            Math.min(36 * 15 - 1, $(window).height() - 30)
          );
        }
      })
  ...
  function menu_toggle(p) {
    ...
    if (p.name == "messagelistmenu") {
      ...
      var fn,
        ...
        target =
          p.props && p.props.link ? p.props.link : p.originalEvent.target;
      ...
      if (p.name == "folder-selector") {
        ...
        p.name == "addressbook-selector" ||
        p.name == "contactgroup-selector"
      ) {
  ...
  function about_dialog(elem) {
    ...
    if (support_link.length && (support_url = support_link.attr("href"))) {
      ...
      support_func = function(e) {
        support_url.indexOf("mailto:") < 0
  ...
  function headers_dialog() {
    var props = { _uid: rcmail.env.uid, _mbox: rcmail.env.mailbox, _framed: 1 },
      dialog = $("<iframe>").attr({
        id: "headersframe",
        src: rcmail.url("headers", props)
      });

    rcmail.simple_dialog(
      dialog,
      rcmail.gettext("arialabelmessageheaders"),
      null,
      {
        cancel_button: "close",
        height: 400
  ...
  function compose_status(id, status) {
    var bar = $("#composestatusbar"),
      ico = bar.find("a.button.icon." + id);

    if (!status) {
      ico.remove();
    } else if (!ico.length) {
      $("<a>")
        .attr("class", "button icon " + id)
        .on("click", function() {
          show_sidebar();
        })
        .appendTo(bar);
  ...
  function attachmentmenu(obj, button, event) {
    ...
    $.each(["open", "download", "rename"], function() {
      var action = this;
      $("#attachmenu" + action, obj)
        .off("click")
        .attr("onclick", "")
        .click(function(e) {
          return rcmail.command(
            action + "-attachment",
            id,
            this,
            e.originalEvent
  ...
  function headersmenu(obj, button, event) {
    $("li > a", obj).each(function() {
      var link = $(this),
        target = "#compose_" + link.data("target");
  ...
  function recipient_selector(field, opts) {
    ...
    rcmail.contact_list.multiselect =
      "multiselect" in opts ? opts.multiselect : true;
  ...
  function update_quota(p) {
    ...
    if (!bar.length) {
      bar = $('<span class="bar"><span class="value"></span></span>').appendTo(
        element
      );
    }
    ...
    bar
      .find(".value")
      .css("width", value + "%")
      [value >= 90 ? "addClass" : "removeClass"]("warning");
    // set title and reset tooltip's data (needed in case of empty title)
    element.attr({
      "data-original-title": "",
      title: element.find(".count").attr("title")
    });
    ...
    if (p.table) {
      element
        ...
        .off("click")
        .on("click", function(e) {
          rcmail.simple_dialog(p.table, "quota", null, {
            cancel_button: "close"
          });
        });
    } else {
      element
        .tooltip("dispose")
        .tooltip({ trigger: is_mobile() ? "click" : "hover" });
    }
  }
  ...
  function recipient_input(obj) {
    var list,
      ...
      insert_recipient = function(name, email, replace) {
        var recipient = $('<li class="recipient">'),
          name_element = $('<span class="name">')
            .html(recipient_input_name(name || email))
            .on("dblclick", function(e) {
              recipient_input_edit_dialog(e, insert_recipient);
            }),
        ...
        recipient
          .attr("title", name ? name + email : null)
          .append([name_element, email_element, link]);

        if (replace) replace.replaceWith(recipient);
        else recipient.insertBefore(input.parent());
      ...
      parse_func = function(e) {
        ...
        if (e.type == "paste") {
          // pasted text
          paste =
            (e.originalEvent.clipboardData || window.clipboardData).getData(
              "text"
            ) || "";
          // insert pasted text in place of the selection (or just cursor position)
          value =
            value.substring(0, this.selectionStart) +
            paste +
      ...
      keydown_func = function(e) {
        ...
        // Here we add a recipient box when the separator (,;) or Enter was pressed
          ...
          e.key == "," ||
          e.key == ";" ||
          (e.key == "Enter" && !rcmail.ksearch_visible())
        ) {
          if (update_func()) {
            return false;
          }
    ...
    input = $("<input>")
      .attr({ type: "text", tabindex: $(obj).attr("tabindex") })
    ...
    $(obj)
      .css({ position: "absolute", opacity: 0, left: "-5000px", width: "10px" })
      .attr("tabindex", -1)
      .after(list)
  ...
  function recipient_input_parser(text) {
    ...
    $.each(matches || [], function() {
      if (
        this.length &&
        (recipient_rx1.test(this) || recipient_rx2.test(this))
      ) {
        var email = RegExp.$1,
  ...
  function image_upload_input(obj) {
    var reset_button = $("<a>")
      ...
      img_onload = function() {
        var state =
          (img.currentSrc || img.src).indexOf(rcmail.env.photo_placeholder) !=
          -1;
  ...
  function iframe_loader(frame) {
    ...
    if (frame.length) {
      var loader = $('<div class="iframe-loader">').append(
  ...
  function pretty_select(select) {
    ...
    var open_func = function(e) {
      var last_char,
        ...
        max_height =
          (document.documentElement.clientHeight || $(document.body).height()) -
          75,
        ...
        var label = $(this).text(),
          link = $('<a href="#">')
            ...
            .addClass(
              this.disabled
                ? "disabled"
                : "active" + (this.value == value ? " selected" : "")
            );
      ...
      select
        ...
        .popover({
          ...
          template:
            ...
            '<div class="popover-body" style="max-height: ' +
            max_height +
            'px"></div></div>'
        })
        .on("shown.bs.popover", function() {
          ...
          if (selected.focus().length) {
            ...
            if (last_index > 5) {
              list_parent.scrollTop(
                list_parent.scrollTop() + list_parent.height() / 2
              );
            }
          }
  ...
  function smart_field_row_add(area, value, name, idx, size, after) {
    ...
    $("a.reset", elem).click(function() {
      ...
      if (area.children().length > 1) {
        $(
          "input",
          record.next().length ? record.next() : record.prev()
        ).focus();
        record.remove();
      } else {
        $("input", record)
  ...
  function switch_nav_list(obj) {
    ...
    if (!navlist.height()) {
      records = $("tr,li", navlist).filter(function() {
        return this.style.display != "none";
      });
      height = $(records[0]).height() || 50;

      navlist.animate(
        { height: Math.min(5, records.length) * height + 1 + "px" },
        speed
      );
  ...
  function splitter_init(node) {
    ...
    var list_id = node
        .find(".scroller .listing")
        .first()
        .attr("id"),
      key =
        rcmail.env.task +
        "." +
        (list_id || rcmail.env.action + "." + node.attr("id")),
      pos = get_pref(key),
          ...
          .on("mousemove.resizer", function(e) {
            ...
            ts = setTimeout(function() {
              ...
              var cursor_position = rcube_event.get_mouse_pos(e).x,
                width = reverted
                  ? node.width() + (offset - cursor_position)
