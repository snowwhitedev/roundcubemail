...
function rcube_webmail()
{
  ...
  // created by al(2020-02-10)
  // go to special step in onboarding modal 
  var goto_step = function(step){
    var data_num = step;
    var w = parseInt($('.containers .modal_content').css("width"));
    if(data_num == 1){
      $('.onboarding_prev_btn').prop("disabled", "disabled");
    }
    else{
      $('.onboarding_prev_btn').prop("disabled", null);
    }
    if(data_num < 5){
      $('.onboarding_next_btn span').text("next");
      $('.onboarding_next_btn i').removeClass("check");
      $('.onboarding_next_btn i').addClass("arrow-right");
    }
    else if(data_num == 5){
      $('.onboarding_next_btn span').text("finish");
      $('.onboarding_next_btn i').removeClass("arrow-right");
      $('.onboarding_next_btn i').addClass("check");
    }
    $('ul li.active').addClass('true').removeClass('active');
    $('ul li[data-num="'+data_num+'"]').addClass('active');
    $('.step span').html("Step "+data_num);
    var margL = -1 * w * (data_num - 1);
    $('.slider-turn').animate({
      'margin-left':margL
    },200);
  };
  ...
  this.init = function()
  {
    ...
    switch (this.task) {
      ...
      case 'mail':
        ...
        // created by al(2020-02-08 onboarding modal)(2020-02-10 updated)
        var prev_cookie = rcmail.get_cookie('prev_cookie');
        var onboarding_cookie = rcmail.get_cookie("onboarding_cookie");
        if(prev_cookie !== null && prev_cookie == 'login' && onboarding_cookie == null){
          // modal expired
          var exp_clear = new Date();
          rcmail.set_cookie("prev_cookie", "", exp_clear);
          // modal create
          var nbP = $('.containers .modal_content').length;
          var w = parseInt($('.containers .modal_content').css("width"));
          var max = (nbP - 1) * w;
          ...
          // goto next step when clicking next button
          $('body').on('click','.onboarding_next_btn', function(){
          ...
          // goto prev step when clicking prev button
          $('body').on('click','.onboarding_prev_btn', function(){
            var margL = parseInt($('.slider-turn').css('margin-left'));
            var modulo = margL%w;
            if (-margL > 0 && modulo == 0) {
              margL += w;
          ...
          // goto special step when clicking step button
          $(".step ul li").on("click", function(){
            var data_num = $(this).attr("data-num");
            goto_step(data_num);
          });

          // goto 2nd step when clicking get started button
          $(".get_start_btn").on("click", function(){
            goto_step(2);
          });

          // goto end step when clicking skip end button
          $(".skip_end").on("click", function(){
            goto_step(5);
          });

          // goto first step when clicking rewind button
          $(".rewind").on("click", function(){
            goto_step(1);
          })

          // close modal when clicking close or bail out button
          $(".bail_out").on("click", function(){
            $('.containers').animate({
              'opacity':0
            },600);
            $('.containers').animate({
        ...
        // don't see again when checked
        $("body").on('change', '#onboarding_chk', function(){
          if($("#onboarding_chk").prop('checked')){
            var exp = new Date();
            exp.setYear(exp.getFullYear() + 1);
            rcmail.set_cookie("onboarding_cookie", "check", exp);
          }
          else{
            var exp = new Date();
            rcmail.set_cookie("onboarding_cookie", "", exp);
          }
        });
      ...
      case 'login':
        ...
        $('form').submit(function () {
          ...
          // created by al(2020-02-09 for onboarding modal)
          var exp = new Date();
          exp.setYear(exp.getFullYear() + 1);
          rcmail.set_cookie("prev_cookie", "login", exp);
          // end
      });
        $("#recover_password").on('click', function(){
          window.open('/manager/users/password-recover.php', '_blank');
        });
        //this.enable_command('recover_password', true);
        break;
    }
  ...
  this.init_messageform = function()
  {
    ...
    if (!html_mode) {
      ...
      console.log('pos', this.env.top_posting);
