    ...
    switch (this.task) {
      ...
      case 'mail':
        ...
        this.env.message_commands = ['show', 'reply', 'reply-all', 'reply-list',
          ...
          'forward', 'forward-inline', 'forward-attachment', 'change-format', "saveaspdf",
          "saveaspwdpdf"];
  ...
  this.command = function(command, props, obj, event)
  {
    ...
    switch (command) {
      ...
      case "saveaspdf":
        ...
        url = this.url(
          "saveaspdf",
          this.params_from_uid(uid, { _safe: this.env.safemode ? 1 : 0 })
        );
        this.open_window(url, true, true);
        break;
      ...
      case "saveaspwdpdf":
        ...
        pdfpwd_dlg =
          '<div id="pdfpwd-input">' +
          '<div><label for="pdfpwd">Password</label>' +
          '<input type="password" id = "pdfpwd"></div>' +
          '<div class="mt-2"><label for="confirmpwd">Confirm</label>' +
          '<input type="password" id="confirmpwd"></div>' +
          "</div>";
        var this_cpy = this;
        this.simple_dialog(
          pdfpwd_dlg,
          "Input your PDF Password",
          function() {
            var pwd = $("#pdfpwd").val();
            pwd = pwd.trim();
            ...
            if (pwd && pwd === confirmpwd) {
              url += "&_pdfpwd=" + pwd;
              this_cpy.open_window(url, true, true);
              return true;
            } else {
              this_cpy.alert_dialog("Please input correct password.");
            }
          },
          {
            button: "Save"
