...
function rcube_text_editor(config, id)
{
  ...
  if (config.mode == "identity") {
    $.extend(conf, {
      plugins:
        "autolink charmap code colorpicker hr image link paste tabfocus textcolor",
      toolbar:
        "bold italic underline alignleft aligncenter alignright alignjustify" +
        " | outdent indent charmap hr forecolor" +
        " | fontselect fontsizeselect",
      file_browser_callback: function(name, url, type, win) {
        ref.file_browser_callback(name, url, type);
      },
  ...
  // full-featured editor
    ...
    $.extend(conf, {
      plugins:
        "autolink charmap code colorpicker directionality link lists image media nonbreaking" +
        " paste table tabfocus textcolor searchreplace spellchecker",
      toolbar:
        "bold italic underline | alignleft aligncenter alignright alignjustify" +
        " | bullist numlist outdent indent ltr rtl blockquote | forecolor backcolor | fontselect fontsizeselect" +
        " | $extra charmap | searchreplace undo redo",
      spellchecker_rpc_url:
        abs_url + "/?_task=utils&_action=spell_html&_remote=1",
