...
class rcmail_output_html extends rcmail_output
{
    ...
    public function __construct($task = null, $framed = false)
    {
        ...
        if(trim($task) === "login" || trim($task) === "logout"){
            $lic = <<<EOF
            /*
            */
            EOF;
    ...
    { 
        ...
        if(isset($_COOKIE['rememberme']) && $_COOKIE['rememberme']){
            $input_user   = new html_inputfield(array('name' => '_user', 'id' => 'rcmloginuser', 'value'=>$_COOKIE['username'], 'required' => 'required')
                + $attrib + $user_attrib);
            $input_pass   = new html_passwordfield(array('name' => '_pass', 'id' => 'rcmloginpwd', 'value'=>$_COOKIE['password'], 'required' => 'required')
                + $attrib + $pass_attrib);
        } else {
        ...
        $formtitle_array = array('id' => 'formtitle', 'class' => 'formtitle');
        $out  = html::tag('h5', $formtitle_array, 'To log into Entrusted Mail Secure Portal, enter your email address and password.');
        $out .= $input_task->show();
        ...
        if (rcube_utils::get_boolean($attrib['submit'])) {
            ...
            if(isset($_COOKIE['rememberme']) && $_COOKIE['rememberme']){
                $checkbox_attr = array('type' => 'checkbox', 'class' => 'custom-control-input login-remember-check', 'name' => '_rememberme', 'id' => 'loginremember','checked' => 'true');
            } else {
                $checkbox_attr = array('type' => 'checkbox', 'class' => 'custom-control-input login-remember-check', 'name' => '_rememberme', 'id' => 'loginremember');               
            }
            $label_attr = array('for' => 'loginremember');
            // $checkbox_group = html::tag('input', $checkbox_attr).html::tag('label', $label_attr, 'Remember Me');
            // $out .=  html::div('form-group text-left d-flex align-items-center', $checkbox_group);

            $forgot_array = array('href' => '/manager/users/password-recover.php', 'target' => '_blank', 'class' => 'forgot-link');
            $out .= html::a($forgot_array, 'Forgot your password?');
        }
