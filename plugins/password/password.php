...
class password extends rcube_plugin
{
    ...
    function password_save()
    {
        ...
        if (($confirm && !isset($_POST['_curpasswd'])) || !isset($_POST['_newpasswd']) || !strlen($_POST['_newpasswd'])) {
            ...
            // password is the same as the old one, warn user, return error
                ...
                $this->rc->output->command('display_message', $res, 'error');
            } else if($newpwd == $_SESSION['username']){
                $this->rc->output->command('display_message', "Cannot match username or email address.", 'error');
            }
    ...
    function password_form()
    {
        ...
        $note_html = "<div class='password-note'>";
        $note_html .= "<p><strong>Password Rules</strong></p>";
        $note_html .= "<p>Cannot match username or email address</p>";
        $note_html .= "<p>Length must be at least 8 characters long.";
        $note_html .= "<p>Must contain both upper case and lower case alphanumeric characters.</p>";
        $note_html .= "<p>Must have at least one numeric character.</p>";
        $note_html .= "<p>Must have at least one special character; such as ~!@#$%^&</p>";
        $note_html .= "</div>";

        return $note_html . html::div(array('id' => 'prefs-title', 'class' => 'boxtitle'), $this->gettext('changepasswd'))
